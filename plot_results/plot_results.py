...
from plot_utils import (
    ...
    set_final_plots, _scale_h, _scale_w, _final_plots_enabled,
    _compute_frontier_indices,
    _collect_top_token_log_probs,
...
do_iwae_vs_k_plot = False

# MARKER_NO_BONUS, MARKER_CFN, MARKER_MIXTURE, MARKER_EXACT_COUNT are imported from plot_utils


def plot_with_conf_bounds(ax, record, x_range, label, bootstrap=False,
                          n_bootstrap_draws=5000, **kwargs):
    """Plot a mean curve with a 95% CI band across seeds (axis 0 of record).

    By default uses a parametric Student-t CI on the mean.  When
    ``bootstrap=True``, uses a percentile bootstrap CI (resampling seeds with
    replacement at each timestep), which respects upper/lower bounds in the
    data — important for quantities with hard bounds such as log probabilities
    (≤ 0).  Falls back to the parametric CI when there are fewer than 2 seeds.
    """
    avg = np.nanmean(record, axis=0)
    n_seeds = record.shape[0]

    if bootstrap and n_seeds >= 2:
        # Per-timestep percentile bootstrap CI of the seed mean.  Naturally
        # respects bounds present in the data (e.g. log probs ≤ 0).
        rng = np.random.default_rng(42)
        boot_means = np.empty((n_bootstrap_draws, record.shape[1]))
        for b in range(n_bootstrap_draws):
            idx = rng.integers(0, n_seeds, size=n_seeds)
            boot_means[b] = np.nanmean(record[idx], axis=0)
        lower_conf_bound = np.nanpercentile(boot_means, 2.5, axis=0)
        upper_conf_bound = np.nanpercentile(boot_means, 97.5, axis=0)
    else:
        stdev = np.nanstd(record, axis=0, ddof=1)
        t_value = stats.t.ppf(0.975, df=n_seeds - 1)
        conf_bound = t_value * stdev / np.sqrt(n_seeds)
    ...
    half_width = (upper_valid[-1] - lower_valid[-1]) / 2
    return avg_valid[-1], half_width
...
def plot_results_over_time(results_list, labels, x_range=None, fontsize=7, figname_modifier="",
                           ...
                           legendfontsize=None, bootstrap_ci=False):
    """
    ...
    Args:
        ...
        bootstrap_ci: If True, use a percentile bootstrap CI instead of the
            parametric Student-t CI.  Recommended for bounded quantities such
            as log probabilities (≤ 0), where the parametric band can otherwise
            extend past the bound.
    """
...
def _compute_and_save_prior_target_kl(kl_results_list, labels, output_dir, file_type_suffix):
    """For the analytic single-token setting, compute and save KL between the prior
    (base actor at t=0) and the target distribution.

    The prior and target are determined by (initial base model, prompt, reward model,
    target_dist_beta) — all assumed fixed across the plotted settings — so the KL is
    computed exactly once. Every loaded entry is verified via torch.allclose; any
    mismatch hard-fails because mixing different priors/targets in one plot is wrong.

    Skips silently only if no entry has the t=0 metrics dict at all (e.g., older save
    format without metrics_list_analytic, or non-analytic setting).
    """
    import json

    # Collect (label, seed_idx, log_p, log_sigma) from every loaded entry. Use try/except
    # only to skip individual entries with an older/different save format — no structural
    # parsing or key sniffing. Verification of cross-entry consistency is done purely via
    # torch.allclose below.
    entries = []
    for label, exp_data in zip(labels, kl_results_list):
        for seed_idx, seed_item in enumerate(exp_data):
            try:
                metrics_t0 = seed_item[2][0]
                log_p = torch.as_tensor(metrics_t0['log_probs_base_full']).float()
                log_sigma = torch.as_tensor(metrics_t0['log_probs_target_full']).float()
            except (TypeError, IndexError, KeyError):
                continue
            entries.append((label, seed_idx, log_p, log_sigma))

    if not entries:
        # No entry had t=0 metrics dicts; not the analytic single-token setting.
        return

    source_label, source_seed_idx, log_p_ref, log_sigma_ref = entries[0]

    # Verify every entry matches the reference. Any failure raises and stops the whole
    # plotting run — if priors or targets disagree across settings, those settings should
    # not have been plotted together in the first place. Tolerance is loose enough to
    # tolerate bf16/fp16 inference jitter (~1e-3) but tight enough to catch real
    # divergences (different beta / model / prompt would produce diffs orders of
    # magnitude larger).
    _atol = 1e-2
    for (lbl, si, log_p, log_sigma) in entries[1:]:
        if not torch.allclose(log_p, log_p_ref, atol=_atol):
            diff = (log_p - log_p_ref).abs()
            raise AssertionError(
                f"Prior (log p at t=0) differs between '{source_label}' (seed {source_seed_idx}) "
                f"and '{lbl}' (seed {si}); these settings should not be plotted together. "
                f"max|diff|={diff.max().item():.3e}, mean|diff|={diff.mean().item():.3e}"
            )
        if not torch.allclose(log_sigma, log_sigma_ref, atol=_atol):
            diff = (log_sigma - log_sigma_ref).abs()
            raise AssertionError(
                f"Target (log sigma at t=0) differs between '{source_label}' (seed {source_seed_idx}) "
                f"and '{lbl}' (seed {si}); these settings should not be plotted together. "
                f"max|diff|={diff.max().item():.3e}, mean|diff|={diff.mean().item():.3e}"
            )

    # KL(p || sigma) = sum_x p(x) * (log p(x) - log sigma(x))
    kl_p_sigma = (torch.exp(log_p_ref) * (log_p_ref - log_sigma_ref)).sum().item()
    # KL(sigma || p) = sum_x sigma(x) * (log sigma(x) - log p(x))
    kl_sigma_p = (torch.exp(log_sigma_ref) * (log_sigma_ref - log_p_ref)).sum().item()

    suffix = f"_{file_type_suffix}" if file_type_suffix else ""
    json_path = os.path.join(output_dir, f"prior_target_kl{suffix}.json")
    with open(json_path, 'w') as f:
        json.dump({
            'kl_p_sigma': kl_p_sigma,
            'kl_sigma_p': kl_sigma_p,
            'computed_from': {'label': source_label, 'seed_index': source_seed_idx},
            'n_entries_verified': len(entries),
        }, f, indent=2)
    print(f"Saved prior-target KL divergences to {json_path}")

    txt_path = os.path.join(output_dir, f"prior_target_kl{suffix}.txt")
    with open(txt_path, 'w') as f:
        f.write("KL between prior (base actor at t=0) and target distribution\n")
        f.write("(analytic single-token setting; computed once)\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"KL(p || sigma):   {kl_p_sigma:.6f}\n")
        f.write(f"KL(sigma || p):   {kl_sigma_p:.6f}\n\n")
        f.write(f"Computed from: '{source_label}' (seed_index={source_seed_idx})\n")
        f.write(f"Verified identical across {len(entries)} (label, seed) entries.\n")
    print(f"Saved prior-target KL summary to {txt_path}")
...
def plot_kl_divergences(file_type_suffix, load_prefixes_to_use, labels, figname_modifier, x_range=None, fontsize=7, n_frontiers=0, legendfontsize=None, n_top_tokens=10, tokenizer_name=None, embedding_pca_path=None, embedding_tsne_path=None):
    ...
    # the already-loaded full-vocab log probs in metrics_list_analytic[0]. NOT wrapped in
    # try/except — if priors/targets disagree across settings, the whole plot is wrong
    # and we want it to fail loudly.
    _compute_and_save_prior_target_kl(kl_results_list, labels, output_dir, file_type_suffix)

    # Frontier plots at training checkpoints (use original prefixes for semantic styling)
    # Plotted first so they are available before the more expensive token-data plots run.
    if n_frontiers > 0:
        _generate_kl_frontier_plots(
    ...
    semantic_colors, semantic_markers, semantic_linestyles = generate_visual_style_from_prefixes(load_prefixes_to_use)
    # Method-level linestyles: one linestyle per method category (CFN vs entropy vs
    # mixture vs annealed beta vs baseline), distinct from the within-hue-group
    # cycling above. Used for coverage curves where methods should be visually
    # distinguishable by linestyle in addition to color.
    method_linestyles = generate_method_linestyles_from_prefixes(load_prefixes_to_use)
    ...
    try:
        plot_coverage_curve(
            ...
            n_top_tokens=n_top_tokens,
            precomputed_token_data=token_data_final,
            linestyle_list=method_linestyles,
        )
    except Exception as e:
    ...
    # Disabled for sampling file type to reduce output
    if file_type_suffix != "sampling":
        try:
...
def _generate_kl_frontier_plots(
    ...
    final_step_title="KL Frontier (final step)",
):
    ...
    last_frontier_t_idx = frontier_indices[-1] if frontier_indices else None
    ...
    for t_idx in frontier_indices:
        ...
        if not skip_individual_timestep_plots:
            frontier_title = final_step_title if t_idx == last_frontier_t_idx else None
...
def plot_iwae_vs_k(f_q_all, g_q_all, labels, figname, fontsize, legendfontsize,
    ...
    K is the total sample count for both bounds:
      LB(K): K proposal samples from q.
      UB(K): 1 exact target sample + (K-1) proposal samples from q.
    ...
    Args:
        f_q_all: _extract_component_per_sample_from_v2_data("f_q_by_prompt_fixed") output.
            List (experiments) of list (seeds) of list (T timesteps) of list (P prompts) of
            numpy array (n_samples,).
        g_q_all: same structure for "g_q_by_prompt_fixed".
        labels: list of experiment label strings.
        figname: output PDF path.
        fontsize: axis/title font size.
        legendfontsize: legend font size.
        color_list: list of colors, one per experiment.
        k_values: list of int total sample counts to evaluate.
    ...
    # In final-plots mode, match the KL frontier plot's figsize (matplotlib defaults,
    # which set_final_plots has already scaled). Otherwise keep the historical size.
    if _final_plots_enabled():
        fig, ax = plt.subplots(figsize=plt.rcParams['figure.figsize'])
    else:
        fig, ax = plt.subplots(figsize=(_scale_w(7), _scale_h(5)))
    has_any = False
    ...
    for exp_i, (exp_label, color) in enumerate(zip(labels, color_list)):
        ...
        print(f"  [IWAE vs K] exp {exp_i} '{exp_label}': {len(f_q_exp)} seed(s)")

        lb_per_seed = []  # list of (k_vals_used_arr, lb_curve) tuples
        ub_per_seed = []

        for seed_j in range(len(f_q_exp)):
            f_q_seed = f_q_exp[seed_j]      # list[T][P] of numpy arrays
            g_q_seed = (g_q_exp[seed_j] if seed_j < len(g_q_exp) else None)
            if not f_q_seed or not g_q_seed:
                print(f"    seed {seed_j}: empty f_q or g_q, skipping")
                continue

            # Select the requested timestep (supports negative indexing)
            n_t = len(f_q_seed)
            t_idx = timestep_to_use if timestep_to_use >= 0 else max(0, n_t + timestep_to_use)
            t_idx = min(t_idx, n_t - 1)
            f_qs_by_prompt = f_q_seed[t_idx]   # list[P] of numpy arrays
            n_g_t = len(g_q_seed)
            g_qs_by_prompt = g_q_seed[min(t_idx, n_g_t - 1)] if n_g_t > 0 else None
            if g_qs_by_prompt is None:
                print(f"    seed {seed_j}: g_q missing at t={t_idx}, skipping")
                continue

            n_prompts = len(f_qs_by_prompt)
            max_k = max(
                (len(fq) for fq in f_qs_by_prompt if fq is not None and len(fq) > 0),
                default=0,
            )
            print(f"    seed {seed_j}: t_idx={t_idx}/{n_t}, {n_prompts} prompts, "
                  f"max samples/prompt={max_k}")
            if max_k == 0:
                continue

            # Cap requested K values to what's available; always include the actual max
            k_vals_capped = sorted(set(
                [k for k in k_values if k <= max_k] +
                ([max_k] if max_k not in k_values else [])
            ))
            print(f"    k_values used: {k_vals_capped}")

            lb_curve, ub_curve = compute_iwae_vs_k_curves(
                f_qs_by_prompt, g_qs_by_prompt,
                k_values=k_vals_capped, n_bootstrap=n_bootstrap, rng_seed=42,
            )
            if lb_curve is None:
                print(f"    seed {seed_j}: no valid prompts for IWAE computation")
                continue

            print(f"    seed {seed_j}: lb range [{lb_curve.min():.3f}, {lb_curve.max():.3f}], "
                  f"ub range [{ub_curve.min():.3f}, {ub_curve.max():.3f}]")
            lb_per_seed.append((np.array(k_vals_capped, dtype=float), lb_curve))
            ub_per_seed.append((np.array(k_vals_capped, dtype=float), ub_curve))

        if not lb_per_seed:
            print(f"  exp {exp_i}: no seeds had valid data, skipping")
            continue

        # Align to the intersection of K values across seeds
        common_k = sorted(set.intersection(*[set(ks.tolist()) for ks, _ in lb_per_seed]))
        if not common_k:
            print(f"  exp {exp_i}: no common K values across seeds, skipping")
            continue
        common_k_arr = np.array(common_k, dtype=float)

        def _align(per_seed_list, ck):
            rows = []
            for ks, curve in per_seed_list:
                ks_list = ks.tolist()
                row = np.array([curve[ks_list.index(k)] for k in ck])
                rows.append(row)
            return np.stack(rows)   # (n_seeds, len(ck))

        lb_matrix = _align(lb_per_seed, common_k)
        ub_matrix = _align(ub_per_seed, common_k)

        has_any = True
        if all_common_k is None:
            all_common_k = common_k_arr
    ...
    # Log-2 x-axis with fixed ticks at the actual K values plotted
    ax.set_xscale('log', base=2)
    tick_positions = all_common_k
    ax.xaxis.set_major_locator(mticker.FixedLocator(tick_positions))
    ax.xaxis.set_major_formatter(mticker.ScalarFormatter())
    ax.xaxis.set_minor_locator(mticker.NullLocator())
    ax.set_xticklabels([str(int(k)) for k in tick_positions], fontsize=fontsize - 2)

    ax.set_xlabel(
        r"Total samples $K$ (LB: $K$ from $q$;  UB: 1 exact $\sigma$ + $K{-}1$ $q$)",
        fontsize=fontsize - 1,
    )
    ax.set_ylabel("SIS/IWAE bound (log $Z$ estimate)", fontsize=fontsize)
    ax.set_title(r"SIS/IWAE bounds vs. number of samples $K$", fontsize=fontsize)
    ax.tick_params(axis='y', labelsize=fontsize - 2)
    ax.grid(True, which='major', alpha=0.3, linestyle='--')

    # Per-experiment legend (solid colored line)
    handles, lgd_labels = ax.get_legend_handles_labels()
    ax.legend(
        handles=handles,
        labels=lgd_labels,
        fontsize=legendfontsize if legendfontsize else fontsize - 2,
        loc='best',
    )
    ...
    plt.clf()
    plt.close(fig)
    print(f"IWAE vs K plot saved to {figname}")
...
def _load_and_reduce_v2_file(path):
    ...
    for key in ("f_q_by_prompt_fixed", "f_q_by_prompt_random",
                "f_q_by_prompt_heldout", "f_q_by_prompt_random_heldout"):
        if key in data:
            data[key] = [
...
def _collect_iwae_bounds_per_prompt(all_v2_data):
    """
    Collect IWAE LB and UB values per prompt, pooled across all experiments,
    seeds, and timesteps.

    Returns:
        prompt_texts: list of prompt text strings (or None if no v2 data found).
        all_lbs_by_prompt: dict prompt_index -> list of all LB values.
        all_ubs_by_prompt: dict prompt_index -> list of all UB values.
    """
    all_lbs_by_prompt = {}
    all_ubs_by_prompt = {}
    prompt_texts = None
    ...
    return prompt_texts, all_lbs_by_prompt, all_ubs_by_prompt


def _collect_seed_averaged_iwae_bounds_per_prompt(all_v2_data):
    """
    For each (experiment, timestep, prompt) cell, average IWAE LB across seeds
    (using non-None values only) and likewise for UB. Return per-prompt lists
    of these seed-averaged values, one entry per (experiment, timestep) cell
    where at least one seed contributed a non-None value.

    Compared to _collect_iwae_bounds_per_prompt (which pools every individual
    seed × timestep value), this collapses the seed axis first via averaging,
    so each pooled scalar is itself a multi-seed mean and thus less sensitive
    to per-seed noise spikes.

    Returns:
        prompt_texts: list of prompt text strings (or None if no v2 data found).
        seed_avg_lbs_by_prompt: dict prompt_index -> list of seed-averaged LB scalars.
        seed_avg_ubs_by_prompt: dict prompt_index -> list of seed-averaged UB scalars.
    """
    seed_avg_lbs_by_prompt = {}
    seed_avg_ubs_by_prompt = {}
    prompt_texts = None
    ...
    for exp_data in all_v2_data:
        ...
        # Determine the largest timestep range across seeds in this experiment
        max_T_lb = 0
        max_T_ub = 0
        for v2_data in exp_data:
            max_T_lb = max(max_T_lb, len(v2_data.get("iwae_lbs_by_prompt_fixed", [])))
            max_T_ub = max(max_T_ub, len(v2_data.get("iwae_ubs_by_prompt_fixed", [])))

        n_prompts = len(prompt_texts)
        for p in range(n_prompts):
            for t in range(max(max_T_lb, max_T_ub)):
                lb_vals = []
                ub_vals = []
                for v2_data in exp_data:
                    iwae_lbs_by_prompt = v2_data.get("iwae_lbs_by_prompt_fixed", [])
                    iwae_ubs_by_prompt = v2_data.get("iwae_ubs_by_prompt_fixed", [])
                    if t < len(iwae_lbs_by_prompt) and p < len(iwae_lbs_by_prompt[t]) \
                            and iwae_lbs_by_prompt[t][p] is not None:
                        lb_vals.append(to_scalar(iwae_lbs_by_prompt[t][p]))
                    if t < len(iwae_ubs_by_prompt) and p < len(iwae_ubs_by_prompt[t]) \
                            and iwae_ubs_by_prompt[t][p] is not None:
                        ub_vals.append(to_scalar(iwae_ubs_by_prompt[t][p]))
                if lb_vals:
                    seed_avg_lbs_by_prompt.setdefault(p, []).append(float(np.mean(lb_vals)))
                if ub_vals:
                    seed_avg_ubs_by_prompt.setdefault(p, []).append(float(np.mean(ub_vals)))

    return prompt_texts, seed_avg_lbs_by_prompt, seed_avg_ubs_by_prompt


def _diagnose_iwae_bounds_per_prompt(all_v2_data, prompts_with_targets, csv_path=None):
    """
    Per-prompt diagnostic on the IWAE LB/UB sandwich. Computes summary statistics
    (mean, std, min, max, median) over all (experiment, seed, timestep) values
    pooled per prompt, plus four candidate log Z midpoints:
      - log_Z_extreme       = (max_LB + min_UB) / 2  over raw (exp, seed, t)
      - log_Z_mean          = (mean_LB + mean_UB) / 2 over raw (exp, seed, t)
      - log_Z_median        = (median_LB + median_UB) / 2 over raw (exp, seed, t)
      - log_Z_seedavg_extreme = (max_LB_seedavg + min_UB_seedavg) / 2 where each
        candidate is itself a seed-average per (exp, t) cell  (currently used by
        _compute_global_per_prompt_log_Z)
    and a noise_ratio = max(std_LB, std_UB) / |gap_mean| to flag prompts where
    sampling noise is large relative to the mean LB/UB gap.

    If csv_path is provided, writes one row per eligible prompt; the table is the
    primary output of this function. A brief summary is also printed.
    """
    prompt_texts, all_lbs_by_prompt, all_ubs_by_prompt = _collect_iwae_bounds_per_prompt(all_v2_data)
    # Also collect the seed-averaged version (one value per (exp, t) cell per prompt)
    _, sa_lbs_by_prompt, sa_ubs_by_prompt = _collect_seed_averaged_iwae_bounds_per_prompt(all_v2_data)
    if prompt_texts is None:
        print("  IWAE bounds diagnostic: no v2 data found")
        return
    ...
    for p in range(len(prompt_texts)):
        if prompt_texts[p] not in prompts_with_targets:
            continue
        lbs = [to_scalar(x) for x in all_lbs_by_prompt.get(p, [])]
        ubs = [to_scalar(x) for x in all_ubs_by_prompt.get(p, [])]
        if not lbs or not ubs:
            continue

        lbs_arr = np.asarray(lbs, dtype=float)
        ubs_arr = np.asarray(ubs, dtype=float)
        mean_lb = float(lbs_arr.mean())
        std_lb = float(lbs_arr.std(ddof=1)) if lbs_arr.size > 1 else 0.0
        min_lb = float(lbs_arr.min())
        max_lb = float(lbs_arr.max())
        median_lb = float(np.median(lbs_arr))
        mean_ub = float(ubs_arr.mean())
        std_ub = float(ubs_arr.std(ddof=1)) if ubs_arr.size > 1 else 0.0
        min_ub = float(ubs_arr.min())
        max_ub = float(ubs_arr.max())
        median_ub = float(np.median(ubs_arr))

        gap_mean = mean_ub - mean_lb
        gap_extreme = min_ub - max_lb
        gap_median = median_ub - median_lb
        noise_ratio = (max(std_lb, std_ub) / abs(gap_mean)) if gap_mean != 0.0 else float("inf")

        # Seed-averaged extremes: one value per (exp, t) cell
        sa_lbs = sa_lbs_by_prompt.get(p, [])
        sa_ubs = sa_ubs_by_prompt.get(p, [])
        if sa_lbs and sa_ubs:
            sa_max_lb = float(max(sa_lbs))
            sa_min_ub = float(min(sa_ubs))
            sa_n_lbs = len(sa_lbs)
            sa_n_ubs = len(sa_ubs)
            sa_gap = sa_min_ub - sa_max_lb
            sa_log_Z = (sa_max_lb + sa_min_ub) / 2.0
        else:
            sa_max_lb = float("nan")
            sa_min_ub = float("nan")
            sa_n_lbs = 0
        ...
        rows.append({
            "prompt_idx": p,
            "prompt_text": prompt_texts[p][:120].replace("\n", " "),
            "n_LBs": int(lbs_arr.size),
            "mean_LB": mean_lb,
            "std_LB": std_lb,
            "min_LB": min_lb,
            "max_LB": max_lb,
            "median_LB": median_lb,
            "n_UBs": int(ubs_arr.size),
            "mean_UB": mean_ub,
            "std_UB": std_ub,
            "min_UB": min_ub,
            "max_UB": max_ub,
            ...
            "log_Z_extreme": (max_lb + min_ub) / 2.0,
            "log_Z_mean": (mean_lb + mean_ub) / 2.0,
            "log_Z_median": (median_lb + median_ub) / 2.0,
            # Seed-averaged-then-extreme (current method)
            "n_LBs_seedavg": sa_n_lbs,
            "n_UBs_seedavg": sa_n_ubs,
            "max_LB_seedavg": sa_max_lb,
            "min_UB_seedavg": sa_min_ub,
            "gap_seedavg_extreme": sa_gap,
            "log_Z_seedavg_extreme": sa_log_Z,
        })

    if not rows:
        print("  IWAE bounds diagnostic: no eligible prompts had both LB and UB values")
        return
    ...
    if csv_path is not None:
        ...
        with open(csv_path, "w", newline="") as f:
            ...
            for row in rows:
                writer.writerow(row)
        print(f"  IWAE bounds diagnostic CSV written to {csv_path} ({len(rows)} prompts)")

    # Brief summary across prompts
    n_inv_extreme = sum(1 for r in rows if r["gap_extreme"] < 0)
    n_inv_mean = sum(1 for r in rows if r["gap_mean"] < 0)
    n_inv_median = sum(1 for r in rows if r["gap_median"] < 0)
    n_with_sa = sum(1 for r in rows if not np.isnan(r["gap_seedavg_extreme"]))
    n_inv_sa = sum(
        1 for r in rows
        if not np.isnan(r["gap_seedavg_extreme"]) and r["gap_seedavg_extreme"] < 0
    )
    nr_arr = np.asarray([r["noise_ratio"] for r in rows], dtype=float)
    finite_nr = nr_arr[np.isfinite(nr_arr)]
    print(
        f"  IWAE bounds diagnostic ({len(rows)} prompts): inverted gaps - "
        f"extreme={n_inv_extreme}/{len(rows)}, mean={n_inv_mean}/{len(rows)}, "
        f"median={n_inv_median}/{len(rows)}, seedavg_extreme={n_inv_sa}/{n_with_sa}"
    )
    if finite_nr.size > 0:
        print(
            f"  noise_ratio = max(std_LB,std_UB)/|gap_mean|: "
            f"mean={finite_nr.mean():.3f}, median={float(np.median(finite_nr)):.3f}, "
            f"min={finite_nr.min():.3f}, max={finite_nr.max():.3f}"
        )


def _compute_global_per_prompt_log_Z(all_v2_data, prompts_with_targets):
    """
    Compute per-prompt log Z from seed-averaged IWAE bounds.

    For each prompt p:
      1. For every (experiment, timestep) cell, average LB across seeds (over
         non-None values only) and likewise for UB. This gives one LB and one
         UB scalar per (method, t) cell.
      2. Pool the seed-averaged LBs over (method, t) and take the max.
         Pool the seed-averaged UBs over (method, t) and take the min.
      3. log_Z_p = (max_LB + min_UB) / 2.

    Compared to taking max/min directly over raw (exp × seed × t) values, this
    should be more stable since each pooled scalar is itself an average over
    seeds rather than a single-seed observation.
    ...
    """
    prompt_texts, all_lbs_by_prompt, all_ubs_by_prompt = \
        _collect_seed_averaged_iwae_bounds_per_prompt(all_v2_data)

    if prompt_texts is None:
        return {}

    log_Z_by_prompt = {}
    # Diagnostics on the IWAE sandwich tightness per prompt.
    # gap = min_UB - max_LB; positive when bounds are consistent (UB above LB),
    # negative when finite-sample noise has produced an inverted sandwich.
    gaps = []
    n_inverted = 0
    n_eligible_prompts = 0  # prompts in prompts_with_targets that had at least one LB and UB
    for p in range(len(prompt_texts)):
        if prompt_texts[p] not in prompts_with_targets:
            continue
        ...
        n_eligible_prompts += 1
        max_lb = to_scalar(max(lbs))
        min_ub = to_scalar(min(ubs))
        gap = min_ub - max_lb
        gaps.append(gap)
        if gap < 0:
            n_inverted += 1
    ...
    if gaps:
        ...
        print(
            f"  log Z IWAE-sandwich diagnostics (seed-averaged then max-LB/min-UB across method×t): "
            f"{n_inverted}/{n_eligible_prompts} prompts inverted (min_UB < max_LB); "
            f"gap (min_UB - max_LB) signed mean={gaps_arr.mean():.4f}, "
            f"min={gaps_arr.min():.4f}, max={gaps_arr.max():.4f}"
        )
    else:
        print("  log Z IWAE-sandwich diagnostics: no eligible prompts had both LB and UB values")
...
def _remap_heldout_to_standard_keys(v2_data):
    """Create a virtual v2 dict where heldout keys are remapped to standard keys.

    This allows reusing _compute_per_prompt_kl_over_time and _compute_random_f_q_over_time
    unchanged for heldout data.
    """
    KEY_MAP = {
        ...
        "log_q_by_prompt_heldout": "log_q_by_prompt_fixed",
        "log_p_by_prompt_heldout": "log_p_by_prompt_fixed",
        "reward_by_prompt_heldout": "reward_by_prompt_fixed",
        "target_by_prompt_heldout": "target_by_prompt_fixed",
        "log_q_g_q_by_prompt_heldout": "log_q_g_q_by_prompt_fixed",
        "log_p_g_q_by_prompt_heldout": "log_p_g_q_by_prompt_fixed",
        "reward_g_q_by_prompt_heldout": "reward_g_q_by_prompt_fixed",
        "target_g_q_by_prompt_heldout": "target_g_q_by_prompt_fixed",
        "f_q_by_prompt_random_heldout": "f_q_by_prompt_random",
        "prompt_texts_heldout": "prompt_texts_fixed",
        "prompt_texts_random_per_timepoint_heldout": "prompt_texts_random_per_timepoint",
        "f_q_estimates_list_heldout": "f_q_estimates_list",
        "g_q_estimates_list_heldout": "g_q_estimates_list",
        "iwae_lbs_list_heldout": "iwae_lbs_list",
        "iwae_ubs_list_heldout": "iwae_ubs_list",
    }
    remapped = {"version": v2_data.get("version", 2)}
    for src_key, dst_key in KEY_MAP.items():
        if src_key in v2_data:
            remapped[dst_key] = v2_data[src_key]
    return remapped


def _has_heldout_data(v2_data):
    """Check if a v2 data dict contains heldout evaluation data."""
    return "f_q_by_prompt_heldout" in v2_data and len(v2_data["f_q_by_prompt_heldout"]) > 0
...
def plot_f_q_g_q_kl_divergences_multiprompt(
    ...
    n_frontiers=1,
    legendfontsize=None,
    preloaded_v2_data=None,
    fig_prefix="",
    heldout_target_samples_path=None,
):
    """
    ...
    # Load v2 files
    if preloaded_v2_data is not None:
        all_v2_data, loaded_seed_indices = preloaded_v2_data
    else:
    ...
    # Detect heldout data: if present and no explicit prefix, use "train_" for this call
    _any_heldout = any(
        _has_heldout_data(v2) for exp_data in all_v2_data for v2 in exp_data
    )
    if _any_heldout and fig_prefix == "":
        fig_prefix = "train_"
    ...
    # Per-prompt IWAE LB/UB diagnostic CSV (mean/std/min/max/median of LB and UB,
    # plus extreme/mean/median candidate log Z midpoints and a noise_ratio).
    _diagnose_iwae_bounds_per_prompt(
        all_v2_data, prompts_with_targets,
        csv_path=os.path.join(per_prompt_dir, f"{fig_prefix}iwae_bounds_per_prompt_diagnostic.csv"),
    )
    ...
    for p in common_prompt_indices if _do_per_prompt_plots else []:
        ...
        for kl_idx, (kl_name, kl_ylabel) in enumerate([
            ...
            plt.tight_layout()
            figname = os.path.join(per_prompt_dir, f"{fig_prefix}prompt_{p:03d}_{sanitized}_{kl_name}.pdf")
            plt.savefig(figname)
            plt.clf()
            plt.close(fig)
    ...
    for kl_idx, (kl_name, kl_ylabel) in enumerate(kl_names_and_ylabels):
        ...
        plt.legend(fontsize=effective_legendfontsize)
        plt.tight_layout()
        figname = os.path.join(per_prompt_dir, f"{fig_prefix}summary_{kl_name}.pdf")
        plt.savefig(figname)
        plt.clf()
        plt.close(fig)
    ...
    _generate_kl_frontier_plots(
        ...
        n_frontiers=n_frontiers, fontsize=fontsize,
        legendfontsize=legendfontsize,
        filename_prefix=fig_prefix,
    )

    # ---- 2b-fq. Frontier: KL(sigma|q) vs random f_q ----
    # Same as combined frontier but with random f_q on the y axis instead of KL(q|sigma).
    has_random_for_frontier = any(
        any(rfq is not None for rfq in exp_rfq) for exp_rfq in all_random_fq
    )
    if has_random_for_frontier:
        mp_fq_frontier_list = []
        for exp_i in range(len(all_summary_kl[1])):
            seed_tuples = []
            n_seeds = len(all_summary_kl[1][exp_i])
            for seed_j in range(n_seeds):
                kl_sigma_q = all_summary_kl[1][exp_i][seed_j] if seed_j < len(all_summary_kl[1][exp_i]) else None
                rfq = all_random_fq[exp_i][seed_j] if seed_j < len(all_random_fq[exp_i]) else None
                if kl_sigma_q is not None and rfq is not None:
                    # Truncate to common length (fixed and random evals may differ)
                    common_len = min(len(kl_sigma_q), len(rfq))
                    seed_tuples.append((kl_sigma_q[:common_len], rfq[:common_len]))
            mp_fq_frontier_list.append(seed_tuples)

        _generate_kl_frontier_plots(
            mp_fq_frontier_list, labels, load_prefixes_to_use, per_prompt_dir,
            n_frontiers=n_frontiers, fontsize=fontsize,
            legendfontsize=legendfontsize,
            filename_prefix=fig_prefix,
            xlabel=r"KL($\sigma$|q)",
            ylabel=r"ELBO (random prompts, mean)",
            plot_name="kl_vs_fq",
            final_step_title=r"KL($\sigma$|q) vs ELBO ($= \log Z - D_{KL}(q\|\sigma)$)",
        )
    ...
    if len(common_prompt_indices) <= 1:
        ...
        if any(len(sd) > 0 for sd in avg_of_avg_results):
            make_frontier_exact_kl_bootstrap(
                ...
                figname=os.path.join(per_prompt_dir, f"{fig_prefix}frontier_kl_avg_of_prompt_avgs.pdf"),
    ...
    if _do_per_prompt_plots:
        ...
        for p in common_prompt_indices:
            ...
            _generate_kl_frontier_plots(
                ...
                n_frontiers=n_frontiers, fontsize=fontsize,
                legendfontsize=legendfontsize,
                filename_prefix=fig_prefix + prefix,
    ...
    if has_random:
        ...
        ax.set_xlabel("Time Steps", fontsize=fontsize)
        ax.set_ylabel(r"ELBO (random prompts, mean)", fontsize=fontsize)
        ax.tick_params(axis='both', labelsize=fontsize)
        if random_f_q_ylim_low is not None:
            ax.set_ylim(bottom=random_f_q_ylim_low)
        plt.legend(fontsize=effective_legendfontsize)
        plt.tight_layout()
        figname = os.path.join(per_prompt_dir, f"{fig_prefix}random_f_q.pdf")
        plt.savefig(figname)
        plt.clf()
        plt.close(fig)
    else:
    ...
    if not individual_prompt_plots or skip_heatmaps:
        ...
        for kl_idx, (kl_name, kl_label) in enumerate([
            ...
            for exp_i in range(len(all_kl_data)):
                ...
                fig, ax = plt.subplots(figsize=(_scale_w(8), _scale_h(fig_height)))
                ...
                figname = os.path.join(per_prompt_dir, f"{fig_prefix}heatmap_{kl_name}{suffix}.pdf")
                plt.savefig(figname)
                plt.clf()
                plt.close(fig)
    ...
    # ---- Heldout eval plots (recursive call) ----
    if _any_heldout and fig_prefix == "train_":
        print("\n=== Generating heldout eval plots ===")
        remapped_v2_data = [
            [_remap_heldout_to_standard_keys(v2) for v2 in exp_data]
            for exp_data in all_v2_data
        ]
        plot_f_q_g_q_kl_divergences_multiprompt(
            load_prefixes_to_use, labels, figname_modifier,
            target_samples_path=heldout_target_samples_path,
            x_range=x_range, fontsize=fontsize, load_dir=load_dir,
            individual_prompt_plots=individual_prompt_plots,
            random_f_q_ylim_low=random_f_q_ylim_low,
            n_frontiers=n_frontiers,
            legendfontsize=legendfontsize,
            preloaded_v2_data=(remapped_v2_data, loaded_seed_indices),
            fig_prefix="eval_",
        )
...
n_top_tokens = 20
tokenizer_name = "distilbert/distilgpt2"
embedding = None  # "embedding_pca_distilgpt2_2p2.pt"
embedding_tsne = None # "embedding_tsne_200_distilgpt2_2p2.pt"
...
# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/ittoxmultitesttoyent | grep f_q | grep _s2); do echo make_list\(\"$x\", 1,10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al0.0001_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al0.0001_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al0.0001_bl0.0_ppq_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al0.0001_bl0.0_ppq_entb0.001_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al0.0001_bl0.0_ppq_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_ppq_entb0.001_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_ppq_entb0.001to0.0linear_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_ppq_entb0.003_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_ppq_entb0.003to0.0linear_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al2e-05_bl0.0_ppq_entb0.0003_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al2e-05_bl0.0_ppq_entb0.001_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al2e-05_bl0.0_ppq_entb0.001to0.0linear_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al2e-05_bl0.0_ppq_entb0.003_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al2e-05_bl0.0_ppq_entb0.003to0.0linear_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al2e-05_bl0.0_ppq_entb0.01_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_entb0.001_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_entb0.001to0.0linear_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_entb0.003to0.0linear_tb200_s2", 1,10),


# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/ittoxmultitesttoyreinf | grep f_q | grep _s2); do echo make_list\(\"$x\", 1,10\)\,; done
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_ppq_mixqi_lag10_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_ppq_mixqi_lag30_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_mixqi_first_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_mixqi_lag10_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_mixqi_lag30_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_ppq_mixqi_lag50_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_p_entb0.001_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al1e-05_bl0.0_p_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_p_entb0.001_tb200_s2", 1,10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al3e-05_bl0.0_p_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al3e-06_bl0.0_p_entb0.001_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al3e-06_bl0.0_p_entb0.003_tb200_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_miprAL_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he10_fs100_scc_al3e-06_bl0.0_p_tb200_s2", 1,10),

]
threshold = -7
figname_modifier = "probinflen20_ittoxmultitesttoy3_04-06_v7"
target_samples_path = None
individual_prompt_plots = False
random_f_q_ylim_low = None
n_frontiers = 4
legendfontsize = 4
n_top_tokens = 1
# #  TODO this is fine, now just fill out all of the remaining seeds
"""
(10, 100)
(10, 100)
(10, 100)
...
# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/ittoxmultitestent | grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done

# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.001to0.0linear_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.003to0.0linear_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.01to0.0linear_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-06_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-06_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-06_bl0.0_ppq_entb0.01_tb250_s2", 1, 10),


# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/ittoxmultitestreinf | grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_first_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_lag10_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_lag30_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_lag50_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_first_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag10_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag30_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag50_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-05_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-05_bl0.0_p_entb0.003_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-06_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-06_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-06_bl0.0_p_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-05_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-05_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-06_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-06_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-06_bl0.0_p_tb250_s2", 1, 10),

# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag10_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag30_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag50_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag30_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),

]
threshold = -7
figname_modifier = "probinflen20_ittoxmulti_04-09_v2"
target_samples_path = None
individual_prompt_plots = False
random_f_q_ylim_low = None
n_frontiers = 4
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestfixed2ent |  grep f_q  |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al0.0001_bl0.0_ppq_tb250_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al0.0001_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.0003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.001to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_entb0.003to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.0003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.001to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.003to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.01_tb250_s2", 1, 10),
#
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestfixed2reinf |  grep f_q  |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_first_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_lag10_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_mixqi_lag30_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag10_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag30_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag50_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-05_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-05_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al1e-05_bl0.0_p_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-05_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-05_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he4_scc_al3e-06_bl0.0_p_tb250_s2", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestfixed2mix |  grep f_q  |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag30_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag50_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag30_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixmx_lag50_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc6.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
#
#
# ]
# threshold = -5
# figname_modifier = "probinflen20_itremodevmulti_b-20_04-09_v2"
# # target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
#
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/dis2reinf/ | grep al3e-06  | grep f_q | grep _s3); do echo make_list\(\"$x\", 1,10\)\,; done
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al0.0001_bl0.0_p_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.0003_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.001_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.003_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-06_bl0.0_p_tb5_s3", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.0003_tb5_s3", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.001_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.003_tb5_s3", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_tb5_s3", 1,10),
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_cf0.1_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1,10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.0003_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.001_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.001to0.0linear_tb5_s3", 1,10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.003_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_entb0.003to0.0linear_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-06_bl0.0_p_tb5_s3", 1,10),
#
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/dis2reinf/ | grep mix  | grep f_q | grep _s3); do echo make_list\(\"$x\", 1,10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixqi_first_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixqi_lag10_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixqi_lag30_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixqi_lag50_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixqi_lag80_tb5_s3", 1,10),
#
#
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/dis2mix/ | grep f_q | grep _s3); do echo make_list\(\"$x\", 1,10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_first_tb5_s3", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_lag10_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_lag30_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_lag50_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_first_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_lag10_tb5_s3", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_lag30_tb5_s3", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_mixmx_lag50_tb5_s3", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc7.0_Sm13In_remodev3lav2_T_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_ppq_tb5_s3", 1,10),
# ]
# figname_modifier = "probinflen20_disitremodev_b-20_04-08_test_v4"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 2
# legendfontsize = 4
...
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestposent |  grep f_q | grep -v 3e-06  |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al0.0001_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al0.0001_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al0.0001_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al0.0001_bl0.0_ppq_entb0.01_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al0.0001_bl0.0_ppq_mixqi_lag10_tb250_s2", 1, 10),
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al0.0001_bl0.0_ppq_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.01_tb250_s2", 1, 10),
#
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestposreinf |  grep f_q | grep -v 3e-06  |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al0.0001_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al0.0001_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al0.0001_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al0.0001_bl0.0_p_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_entb0.001to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_entb0.003to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_entb0.0003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_entb0.001to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_entb0.003_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestposmix |  grep f_q |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_mixmx_lag10_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_mixmx_lag30_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_mixmx_lag10_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctln_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_mixmx_lag30_tb250_s2", 1, 10),
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# # # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestposreinf2 |  grep f_q | grep -v 3e-06  |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_divab_tb250_s2", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen20_itremodevmultitestpos_b20_04-09_v2"
# # target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 8
...
#
# # for x in $(ls info/exploretoyrlhfmulti03v6 | grep analy |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_entb0.003_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_entb0.01_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_entb0.03_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_entb0.1_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_entb0.3_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_p_tb5_s2", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen1_exploretoyrlhfmulti03_04-05_v7"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
#
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/toy2p2len4reinf/ | grep f_q | grep _s2); do echo make_list\(\"$x\", 1,10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al0.0001_bl0.0_p_tb5_s2", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_cf10.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_cf3.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.001_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.003_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.003to0.0linear_tb5_s2", 1,10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.01_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_entb0.01to0.0linear_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al1e-05_bl0.0_p_tb5_s2", 1,10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc9.0_di_To_2_l4_kl0.0_b-10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-05_bl0.0_p_tb5_s2", 1,10),
#
# ]
# figname_modifier = "probinflen4_toytox_2p2_b10_04-05_v7"
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al1e-05_bl0.0_ppq_tb250_s5", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s5", 1, 10),
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s5", 1, 10),
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.001_tb250_s5", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_entb0.003_tb250_s5", 1, 10),
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_s-5.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_tb250_s5", 1, 10),
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag10_tb250_s5", 1, 10),
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rc10.0_Sm13In_To_20misi1_l20_kl0.0_b-20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he4_scc_al3e-05_bl0.0_ppq_mixqi_lag50_tb250_s5", 1, 10),
#
# ]
# threshold = -7
# figname_modifier = "probinflen20_ittoxmultifixedeval_b-20_04-17_v2"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
#
load_prefixes_to_use = [
# for x in $(ls /scratch/zhaostep/OpenRLHF/info/probinfrlhfmultifixedeval/ |  grep f_q | grep _s5  ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_entb0.003_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_mixqi_lag10_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_mixqi_lag30_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al5e-07_bl0.0_ppq_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s10.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_tb80_s5", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s30.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_tb80_s5", 1, 10),

# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_mixqi_lag10_tb80_s1", 1, 10),
...
# cd /h/319/stephenzhao/OpenRLHF/info/probinfrlhfmultifixedeval
# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/probinfrlhfmultifixedeval |  grep f_q | grep 80_s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_entb0.003_tb80_s1", 1, 10),
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb80_s1", 1, 10),
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s10.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_tb80_s1", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s10.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_tb80_s1", 1, 10),
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s30.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-06_bl0.0_ppq_tb80_s1", 1, 10),

]

threshold = -5
figname_modifier = "probinflen100_probinfrlhfmulti_b50_04-17_v2"
# target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
individual_prompt_plots = False
random_f_q_ylim_low = None
n_frontiers = 5
...
#
#
# # make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_ppq_ctl_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_ppq_mixqi_first_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_ppq_ctl_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_ppq_mixqi_lag50_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_s-0.03_b-1.0_hlnt_a0.0_ppq_ctl_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_s-0.1_b-1.0_hlnt_a0.0_ppq_ctl_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_ppq_tb5_s2", 1, 10),
...
# ]
# threshold = -5
# figname_modifier = "probinflen1_tox2p2_04-21"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al1e-05_bl0.0_ppq_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl0.0_ppq_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.003to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.01to0.0linear_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_mixmx_lag10_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_mixqi_lag10_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
#
#
# # for x in $(ls info/itremodevmultitestposfixedevalannealbeta/ |  grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s0.3_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s1.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_tb250_s2", 1, 10),
...
#
# # for x in $(ls info/itremodevmultitestposfixedevalcombined/ | grep -v 0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_entb0.0003_ | grep -v s5.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.1 | grep -v s10.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.1 | grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.1_cd64_cfr0.001_cfsn_af_fo_entb0.0001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_entb0.0003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_entb0.0001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s10.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s10.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.0001_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s10.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.0003_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.0001_tb250_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl0.0_ppq_entb0.0003_tb250_s2", 1, 10),
#
#
#
# ]
# threshold = -5
# figname_modifier = "probinflen20_itremodevmultitestposfixedeval_b20_04-19_v4"
# # target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 8
...
# for x in $(ls info/repulselen20 | grep info_eval | grep -v entb0.003 | grep -v entb0.01 | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-10.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_entb0.0003_tb250_s2", 1, 10),
make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-10.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-10.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_cf0.5_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_cf2.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_entb0.0001_tb250_s2", 1, 10),
make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_entb0.0003_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_entb0.001_tb250_s2", 1, 10),
make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al1e-05_bl3e-05_ppq_entb0.0001_tb250_s2", 1, 10),
make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al1e-05_bl3e-05_ppq_entb0.0003_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al1e-05_bl3e-05_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_entb0.0003_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_entb0.001_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-3.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-10.0_b-30.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al1e-05_bl3e-05_ppq_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-10.0_b-30.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s2", 1, 10),
make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-3.0_b-30.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s2", 1, 10),


# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/repulselen20extra | grep info_eval | grep _s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-1.0_b-10.0_sb-10.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s1", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-1.0_b-20.0_sb-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s1", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-3.0_b-10.0_sb-10.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s1", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-3.0_b-10.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s1", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-3.0_b-20.0_sb-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s1", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_s-3.0_b-20.0_sb-30.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-05_bl3e-05_ppq_tb250_s1", 1, 10),

# for x in $(ls /h/319/stephenzhao/OpenRLHF/info/rlhfmultilen20kl2v2 | grep mix | grep b-20 | grep info_eval | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_mixqi_first_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_mixqi_lag30_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_mixqi_lag50_tb250_s2", 1, 10),
# make_list("info_eval_rlhf_Sm13In_remodev3lav2_20misi1_l20_kl0.2_b-20.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al2e-05_bl3e-05_ppq_mixqi_lag80_tb250_s2", 1, 10),
...
threshold = -5
figname_modifier = "repulsemultilen20kl2v2_04-21_v4"
target_samples_path = None
individual_prompt_plots = False
random_f_q_ylim_low = None
n_frontiers = 2
...
# load_prefixes_to_use = [
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/tinystoriesbounds | grep -v al3e-05 | grep -v al1e-05 | grep -v al3e-07 | grep f_q_ | grep _s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al0.0001_bl0.0_ppq_tb1000_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_entb0.0003_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_entb0.001_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-05_bl0.0_ppq_tb5_s1", 1, 10),
#
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.0003_tb5_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.001_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.01_tb10_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.03_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb10_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb40_s1", 1, 10),
# #
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al0.0001_bl0.0_ppq_tb1000_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-05_bl0.0_ppq_tb1000_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-05_bl0.0_ppq_tb1000_s1", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/tinystoriesbounds | grep tb5 | grep f_q_ | grep _s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-05_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-05_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-05_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
#
#
# # # # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/tinystoriesbounds | grep -v tb5 | grep f_q_ | grep _s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al0.0001_bl0.0_ppq_tb1000_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-05_bl0.0_ppq_tb1000_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-05_bl0.0_ppq_tb1000_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb10_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb160_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb20_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb40_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.003_tb80_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_entb0.01_tb10_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_tb10_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_tb160_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_tb20_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_tb40_s1", 1, 10),
# # # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_tb80_s1", 1, 10),
#
#
# ]
# threshold = -5
# figname_modifier = "boundslen10_04-20_v10_b5"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# load_prefixes_to_use = [
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/tinystoriesbounds | grep he100 | grep tb5 | grep f_q_ | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-05_bl0.0_ppq_entb0.003_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-05_bl0.0_ppq_entb0.003_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
...
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/tinystoriesbounds2 | grep f_q_ | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf3.0_cd64_cfr0.003_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_ppq_cf3.0_cd64_cfr0.0001_cfsn_af_fo_tb5_s2", 1, 10),
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_first_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_lag100_tb5_s2", 1, 10),
...
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_lag200_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_lag30_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_lag400_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_lag50_tb5_s2", 1, 10),
#
#
# ]
# threshold = -5
# figname_modifier = "boundslen10_04-22_v3"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# # make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.2_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_tb80_s1", 1, 10),
#
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/repulselen100 | grep info_ | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_cf1.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_entb0.0003_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_entb0.001_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_entb0.003_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.2_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_cf1.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_b-5.0_hlnt_a0.2_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_s-0.5_b-5.0_sb-5.0_hlnt_a0.1_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_tb80_s2", 1, 10),
# make_list("info_eval_rlhf_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl2.0_s-0.5_b-5.0_sb-5.0_hlnt_a0.2_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl3e-07_ppq_tb80_s2", 1, 10),
# ]
# threshold = -7
# figname_modifier = "repulselen100_04-23"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# ]
# threshold = -5
# figname_modifier = "boundslen10_04-28_final_wmixture"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# ]
# threshold = -5
# figname_modifier = "boundslen10_04-28_final"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s1", 1, 10),
#
# # /scratch/zhaostep/OpenRLHF/info/probinfrlhfmultifixedevalextra
# # for x in $(ls  /scratch/zhaostep/OpenRLHF/info/probinfrlhfmultifixedevalextra |  grep f_q | grep s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr1e-05_cfsn_af_fo_tb80_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr3e-05_cfsn_af_fo_tb80_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr3e-06_cfsn_af_fo_tb80_s1", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen100_probinfrlhfmulti_b50_04-24"
# # target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 2
...
# load_prefixes_to_use = [
# # # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/probinflen1remodevpos | grep fs100 | grep -v s0.3_b20.0 | grep -v cf10 | grep -v cf0.3 | grep -v entb0.001 | grep -v s10.0_b20 | grep analy |  grep _s5 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.03_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s1.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/probinflen1remodevpos | grep fs100 | grep 1e-06 | grep -v s0.3_b20.0 | grep -v cf10 | grep -v cf0.3 | grep -v entb0.001 | grep -v s10.0_b20 | grep analy |  grep _s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.03_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s1.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
#
#
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b10.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s5", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s5", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf3.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s5", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/probinflen1remodevposmix | grep analy |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_first_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag10_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag120_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag30_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag50_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag80_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag200_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_mixqi_lag400_tb5_s2", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/probinflen1remodevposcombined | grep analy |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
#
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_entb0.003_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_entb0.01_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_entb0.003_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_entb0.01_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_entb0.01_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.003_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b20.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.01_tb5_s2", 1, 10),
#
#
# ]
# threshold = -5
# figname_modifier = "probinflen1_remodevthis_b20_04-30"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# load_prefixes_to_use = [
# # # for x in $(ls /scratch/zhaostep/OpenRLHF/info/probinflen1remodevpos50 | grep analy | grep 5_s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-07_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s10.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s10.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# ]
# threshold = -5
# figname_modifier = "probinflen1_remodevthis_b30_04-50_v2"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# load_prefixes_to_use = [
# # # for x in $(ls /scratch/zhaostep/OpenRLHF/info/probinflen1remodevpos100 | grep analy | grep 5_s1 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.003_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.01_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-07_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s10.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s10.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s1", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b100.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s1", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen1_remodevthis_b30_04-100_v2"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# load_prefixes_to_use = [
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/indalertt3 | grep al3e-07 | grep -v cf30 | grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al1e-06_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al1e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al1e-06_bl0.0_ppq_tb20_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_cf30.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_tb20_s1", 1, 10),
#
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_cf10.0to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s1", 1, 10),
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_entb0.001_tb20_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_entb0.003_tb20_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_entb0.01_tb20_s1", 1, 10),
#
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it0.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it3.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it5.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it8.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he200_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen100_indremodev_-3_05-01_v11"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 11
...
# load_prefixes_to_use = [
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/indalertt3_100 | grep al3e-07 |  grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it0.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_entb0.003_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it3.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it5.0to-3.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_tb20_s2", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen100_indremodev_-3_he100_05-02_v2"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 10
...
# load_prefixes_to_use = [
# # # for x in $(ls /scratch/zhaostep/OpenRLHF/info/probinflen1remodevpos30 | grep analy | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf10.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf3.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.003_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.01_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.03_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.03to0.0linear_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.1_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_entb0.1to0.0linear_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
#
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf10.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_cf3.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.003_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.01_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.03_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.03to0.0linear_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.1_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_entb0.1to0.0linear_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-07_bl0.0_ppq_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s0.3_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s0.3_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s10.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s10.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s2", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s1.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s1.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al1e-06_bl0.0_ppq_tb5_s2", 1, 10),
# # make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s3.0_b30.0_hlnt_a0.0_ppq_ctl_ep1_e1_he20_fs100_scc_al3e-06_bl0.0_ppq_tb5_s2", 1, 10),
# ]
# threshold = -5
# figname_modifier = "probinflen1_remodevthis_b30_05-02"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 10
...
threshold = -5
figname_modifier = "probinflen100_indremodev_-5_he100_allstuff_05-06_test"
target_samples_path = None
individual_prompt_plots = False
random_f_q_ylim_low = None
n_frontiers = 9
...
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/itremodevmultitestposfixedevalreinf |  grep f_q |  grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al0.0001_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf0.3to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s10.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_divab_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s10.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s1.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s3.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al1e-05_bl0.0_p_tb250_s2", 1, 10),
make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_s5.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_tb250_s2", 1, 10),

make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),

# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap5.0_Sm13In_remodev3lav2_20misi1_l20_kl0.0_b20.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-05_bl0.0_p_cf3.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb250_s2", 1, 10),


]
threshold = -5
figname_modifier = "probinflen20_itremodevmultitestpos_b20_wreinf_05-25_final"
# target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
individual_prompt_plots = False
random_f_q_ylim_low = None
n_frontiers = 8
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_scc_al3e-07_bl0.0_ppq_cf10.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
#
# # # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/indalertt5_100_reinf | grep al3e-07 | grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
#
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it0.0to-5.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-3.0to-5.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it3.0to-5.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf10.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf10.0to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf30.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf30.0to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf3.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf3.0to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it5.0to-5.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it8.0to-5.0linear_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_tb20_s2", 1, 10),
# # TODO CHECK MORE RESULTS HERE
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf15.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf20.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf5.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ll3.1BIn_SkReV2Ll3.1B_BaALseshen_l100_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_scc_al3e-07_bl0.0_p_cf8.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb20_s2", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen100_indremodev_-5_he100_wreinf_05-13_final"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 8
# legendfontsize = 9
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_ppq_ctl_ep1_e1_he2_scc_al3e-07_bl0.0_ppq_cf0.3to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s1", 1, 10),
#
# # for x in $(ls /scratch/zhaostep/OpenRLHF/info/probinfrlhfmultifixedevalreinf/ | grep al3e-07 | grep f_q | grep _s2 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_cf0.3_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_cf0.3to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_cf1.0to0.0linear_cd64_cfr0.001_cfsn_cfpSm13In_af_fo_tb80_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_tb80_s1", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s10.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_tb80_s1", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_rlhf_rcap8.0_Ll3.1BIn_SkReV2Ll3.1B_20misi1_l100_kl0.0_s20.0_b50.0_hlnt_a0.0_p_reinf_ep1_e1_he2_scc_al3e-07_bl0.0_p_tb80_s1", 1, 10),
#
# ]
#
# threshold = -5
# figname_modifier = "probinflen100_probinfrlhfmulti_b50_wreinf_05-13_final"
# # target_samples_path = "info/target_samples_Sm13In_remodev3lav2_rlhf_l20_b-20.0_rc6.0_20misi1_tsa20.pt"
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_ppq_ctl_ep1_e1_he100_fs50_scc_al1e-06_bl0.0_ppq_mixqi_lag10_tb5_s2", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/tinystoriesboundsreinf | grep al3e-06 | grep f_q | grep _s3 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it0.0to-5.0linear_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it3.0to-5.0linear_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf100.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf10.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf1.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf30.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_tb5_s2", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it5.0to-5.0linear_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_tb5_s2", 1, 10),
#
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf100.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf20.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf30.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf50.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf50.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1, 10),
# make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf70.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1, 10),
# # make_list("f_q_g_q_iwae_bounds_OpenRLHF_it-5.0_Ti33_To_O_l10_kl0.0_b1.0_hlnt_a0.0_p_reinf_ep1_e1_he100_fs50_scc_al3e-06_bl0.0_p_cf80.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s3", 1, 10),
#
#
# ]
# threshold = -5
# figname_modifier = "boundslen10_wreinf_05-13_final"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
#
#
# # # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/probinflen1remodevposreinf | grep analy | grep al3e-05 | grep b10.0 | grep _s9 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs100_scc_al3e-05_bl0.0_p_cf1.0to0.0linear_cd64_cfr0.001_cfsn_af_fo_tb5_s9", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_b10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs100_scc_al3e-05_bl0.0_p_tb5_s9", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_remodev3lav2_thmaisa_l1_kl0.0_s1.0_b10.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs100_scc_al3e-05_bl0.0_p_tb5_s9", 1, 10),
#
# ]
# threshold = -5
# figname_modifier = "probinflen1_remodevthis_b10_wreinf_05-13_final"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_ppq_ctl_ep1_e4_he5_fs50_scc_al3e-05_bl0.0_ppq_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s2", 1, 10),
#
# # for x in $(ls /h/319/stephenzhao/OpenRLHF/info/exploretoyrlhfmulti03v5reinf | grep al3e-05 | grep analy |  grep _s9 ); do echo make_list\(\"$x\", 1, 10\)\,; done
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-05_bl0.0_p_cf3.0_cd64_cfr0.001_cfsn_af_fo_tb5_s9", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_b-1.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-05_bl0.0_p_tb5_s9", 1, 10),
# make_list("analytic_kls_toxicity_rlhf_di_To_2_l1_kl0.0_s-0.3_b-1.0_hlnt_a0.0_p_reinf_ep1_e1_he20_fs50_scc_al3e-05_bl0.0_p_tb5_s9", 1, 10),
#
#
# ]
# threshold = -5
# figname_modifier = "probinflen1_tox2p2_05-04_wreinf_05-13_final"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
# ]
# threshold = -5
# figname_modifier = "probinflen20_itremodevmultitestposfixedeval_b20_05-04_final_wmoretoptokens"
# target_samples_path = None
# individual_prompt_plots = False
# random_f_q_ylim_low = None
# n_frontiers = 4
...
if use_info_eval:
    ...
    # Use the multiprompt v2 path (works with or without target_samples_path).
    # Previously we also called the aggregated-format path here for backward compat,
    # but it produced frontier_kl_*.pdf with the same filenames as the multiprompt path
    # using a different (less principled) global log Z, silently overwriting them.
    ...
    # Method-level linestyles for coverage curves (one per method category).
    _method_linestyles = generate_method_linestyles_from_prefixes(load_prefixes_to_use)
    ...
    # IWAE bounds vs number of samples N (first, since it's the primary new plot).
    # f_q_by_prompt_fixed stores the aggregated scalar IWAE LB, not per-sample weights.
    # The individual per-sample log importance weights must be reconstructed from components:
    #   f_q per sample = target_by_prompt_fixed - log_q_by_prompt_fixed
    #   g_q per sample = target_g_q_by_prompt_fixed - log_q_g_q_by_prompt_fixed
    if do_iwae_vs_k_plot:
        try:
            target_fq = _get_component("target_by_prompt_fixed")
            log_q_fq  = _get_component("log_q_by_prompt_fixed")
            target_gq = _get_component("target_g_q_by_prompt_fixed")
            log_q_gq  = _get_component("log_q_g_q_by_prompt_fixed")

            def _subtract_components(a_all, b_all):
                """Compute a[exp][seed][t][p] - b[exp][seed][t][p] element-wise."""
                result = []
                for exp_a, exp_b in zip(a_all, b_all):
                    exp_res = []
                    for seed_a, seed_b in zip(exp_a, exp_b):
                        seed_res = []
                        for t_a, t_b in zip(seed_a, seed_b):
                            t_res = []
                            for p_a, p_b in zip(t_a, t_b):
                                if p_a is None or p_b is None:
                                    t_res.append(None)
                                else:
                                    t_res.append(p_a - p_b)
                            seed_res.append(t_res)
                        exp_res.append(seed_res)
                    result.append(exp_res)
                return result

            f_q_weights = _subtract_components(target_fq, log_q_fq)
            g_q_weights = _subtract_components(target_gq, log_q_gq)

            plot_iwae_vs_k(
                f_q_all=f_q_weights,
                g_q_all=g_q_weights,
                labels=labels,
                figname=os.path.join(_output_dir, "sampling_iwae_vs_k.pdf"),
                fontsize=fontsize,
                legendfontsize=_lfs,
    ...
    try:
        ...
        if has_counts:
            plot_vocab_coverage_from_history(
                ...
                color_list=_semantic_colors, fontsize=fontsize, legendfontsize=_lfs,
                marker_list=_semantic_markers, linestyle_list=_method_linestyles,
            )
    ...
    try:
        ...
        if has_data:
            ...
            plot_two_series_lollipop(
                ...
                series3_name=r'$\sigma$ (log Z estimate)',
