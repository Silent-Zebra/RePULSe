...
PARAMS=$(echo "$COMMAND" | awk -v pretrain_logic="$PRETRAIN_LOGIC" '
{
    ...
    exploration_bonus_sa = exploration_bonus_ba = bonus_alpha = coin_flip_dim = coin_flip_lr = coin_flip_norm_momentum = coin_flip_update_steps = coin_flip_head_init_std = frozen_prior_init_std = coin_flip_linear_bias = coin_flip_architecture = coin_flip_pretrain = analytic_batch = train_coin_flip_before = coin_flip_first_online =     coin_flip_use_prioritization = coin_flip_warmup_steps = reward_clamp = reward_cap = rejection_sample_true_target_only = ""
    ...
    print micro_train "|" train "|" micro_rollout "|" rollout "|" max_epochs "|" epi_str "|" \
          ...
          reward_pretrain "|" prompt_data "|" init_head_from_base "|" sd_divider "|" harmloss "|" harmlossreinbaseline "|" hlrbval "|" alpha "|" kl "|" only_eval_neg "|" use_base_as_proposal "|" rta "|" rtb "|" startb "|" starta "|" sepb "|" uniw "|" exploration_bonus_sa "|" exploration_bonus_ba "|" bonus_alpha "|" coin_flip_dim "|" coin_flip_lr "|" coin_flip_norm_momentum "|" coin_flip_update_steps "|" coin_flip_head_init_std "|" frozen_prior_init_std "|" coin_flip_linear_bias "|"           coin_flip_architecture "|" coin_flip_pretrain "|" analytic_batch "|" train_coin_flip_before "|" coin_flip_first_online "|" coin_flip_use_prioritization "|" coin_flip_warmup_steps "|" reward_clamp "|" reward_cap "|" rejection_sample_true_target_only "|" mixture_proposal "|" mixture_optimization "|" mixture_other_model "|" mixture_lag_steps "|" mixture_psi_use_mix "|" load_traj \
}')
...
IFS='|' read MICRO_TRAIN TRAIN MICRO_ROLLOUT ROLLOUT MAX_EPOCHS EPI_STR GEN_MAX_LEN \
    ...
    INITHEADBASE SD_DIVIDER HARMLOSS HARMLOSSREINBASELINE HLRBVAL ALPHA KL ONLY_EVAL_NEG BASE_PROP RTA RTB STARTB STARTA SEPB UNIW EXPLORATION_BONUS_SA EXPLORATION_BONUS_BA BONUS_ALPHA COIN_FLIP_DIM COIN_FLIP_LR COIN_FLIP_NORM_MOMENTUM COIN_FLIP_UPDATE_STEPS COIN_FLIP_HEAD_INIT_STD FROZEN_PRIOR_INIT_STD COIN_FLIP_LINEAR_BIAS COIN_FLIP_ARCHITECTURE COIN_FLIP_PRETRAIN ANALYTIC_BATCH TRAIN_COIN_FLIP_BEFORE COIN_FLIP_FIRST_ONLINE COIN_FLIP_USE_PRIORITIZATION COIN_FLIP_WARMUP_STEPS REWARD_CLAMP REWARD_CAP EXACTTARGET MIXTURE_PROPOSAL MIXTURE_OPTIMIZATION MIXTURE_OTHER_MODEL MIXTURE_LAG_STEPS MIXTURE_PSI_USE_MIX LOAD_TRAJ <<< "$PARAMS"
...
PATTERN="${CURRENT_DATE}${ONLY_EVAL_NEG}_${PRETRAIN}_${REWARD_PRETRAIN}_${PROMPT_DATA}_${RM_TYPE}${BASE_PROP}${EXACTTARGET}${THRESH}${STARTB}${TARGET_BETA}${SEPB}${UNIW}${KL}_len${GEN_MAX_LEN}_${PARAMETERIZATION}${INITHEADBASE}${SD_DIVIDER}_b${MICRO_TRAIN}_${TRAIN}${ANALYTIC_BATCH}_${MICRO_ROLLOUT}_${ROLLOUT}${DUP_ROLLOUT}_epo${MAX_EPOCHS}${EPI_STR}${HARMLOSS}${HARMLOSSREINBASELINE}${HLRBVAL}${STARTA}${ALPHA}${RTA}${RTB}${BASEACTOR_LR}_${ACTOR_LOSS}${ACTOR_LR}${CRITIC_LR}_${LR_SCHED}${CUSTOM_PROMPT}${SAVE_NEGDATA_THRESH}${EXPLORATION_BONUS_SA}${EXPLORATION_BONUS_BA}${BONUS_ALPHA}${COIN_FLIP_DIM}${COIN_FLIP_LR}${COIN_FLIP_NORM_MOMENTUM}${COIN_FLIP_UPDATE_STEPS}${COIN_FLIP_HEAD_INIT_STD}${FROZEN_PRIOR_INIT_STD}${COIN_FLIP_LINEAR_BIAS}${COIN_FLIP_ARCHITECTURE}${COIN_FLIP_PRETRAIN}${TRAIN_COIN_FLIP_BEFORE}${COIN_FLIP_FIRST_ONLINE}${COIN_FLIP_USE_PRIORITIZATION}${COIN_FLIP_WARMUP_STEPS}${REWARD_CLAMP}${REWARD_CAP}${MIXTURE_PROPOSAL}${MIXTURE_OPTIMIZATION}${MIXTURE_OTHER_MODEL}${MIXTURE_LAG_STEPS}${MIXTURE_PSI_USE_MIX}${LOAD_TRAJ}"
